...
public class QiqiSearch {
	...
	public void init_words() {
		...
		words[11] = "其它运动激选择微如熊加参刺猬辣椒填晚傍漂流哪棵迎接粗细桃核泪种结束摘梨错会拉差话力抓此苦新闻行粮食瓶";
	...
	public void init_can_read_words() {
		...
		can_read_words += "课题糕苹太饭入鸡爬树幼儿园换种玩会拿猪狗杂每兔本领孩娃没有发芽宝";
		...
		while (true) {
			...
			for (int i = 0; i < input.length(); i++) {
				...
				if (search.dict_words.get(input.charAt(i)) != null) {
					...
					String output = String.format("%c    %s    %s", input.charAt(i), "null", can_read);
					System.out.println(output);
				}
			}
			System.out.println("");
